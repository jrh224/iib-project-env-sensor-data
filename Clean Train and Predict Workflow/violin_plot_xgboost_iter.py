...
model_filepath = "optimal_singleoutput_xgb_model.pkl"
...
for i_start in range(0, test_matrix.shape[0] - config.LOOKBACK - config.HORIZON, config.STRIDE): # stride = 1
    y_prediction = xgboost_autoregressive_predict(loaded_model, test_matrix, config.HORIZON, i_start)
...
fontsize = 15
labelsize = 13
plt.title("MAE Distribution Across Timesteps (XGBoost Iterative)", fontsize=fontsize)
