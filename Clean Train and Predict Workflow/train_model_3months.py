...
X_train, y_train = create_sequences(train_matrix, lookback=config.LOOKBACK, predictforward=config.OUTPUT_SIZE, step=config.SEQ_STEP, target_col=0, blocks=idx_blocks_train)
X_val, y_val = create_sequences(val_matrix, lookback=config.LOOKBACK, predictforward=config.OUTPUT_SIZE, step=config.SEQ_STEP, target_col=0, blocks=idx_blocks_val)

print(f"Training ({X_train.shape[0]}) and validation ({X_val.shape[0]}) sequences created successfully")
...
best_model_path = '26feb1433.pth'  # Path to save best model
